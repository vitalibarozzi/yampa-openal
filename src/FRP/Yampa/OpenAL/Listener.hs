    ...
    liftIO do
        case mlistener0 of
            Nothing -> do
                ...
                ($=) AL.listenerGain (abs (realToFrac $ listenerGain (listener1)))
            Just listener0 -> do
                ($=?) AL.listenerPosition True (_v3ToVertex (listenerPosition $ listener1))
                ($=?) AL.listenerVelocity True (_v3ToVector (listenerVelocity $ listener1))
                ($=?) AL.orientation True (bimap _v3ToVector _v3ToVector (listenerOrientation $ listener1))
                ($=?) AL.listenerGain True (abs (realToFrac $ listenerGain (listener1)))
